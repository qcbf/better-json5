...
namespace SchemaAssociationNotification {
	export const type: NotificationType<ISchemaAssociations | SchemaConfiguration[]> = new NotificationType('json5/schemaAssociations');
}
...
namespace DocumentSortingRequest {
	export const type: RequestType<DocumentSortingParams, TextEdit[], any> = new RequestType('json5/sort');
}
...
export function startServer(connection: Connection, runtime: RuntimeEnvironment) {
	...
	let colorDecoratorLimit = Number.MAX_VALUE;
	...
	connection.onDocumentColor((params, token) => {
		return runSafeAsync(runtime, async () => {
			...
			if (document) {
				...
				const jsonDocument = getJSONDocument(document);
				const resultLimit = colorDecoratorLimit;
	...
	connection.onFoldingRanges((params, token) => {
		return runSafe(runtime, () => {
			const document = documents.get(params.textDocument.uri);
			if (document) {
				const rangeLimit = foldingRangeLimit;
